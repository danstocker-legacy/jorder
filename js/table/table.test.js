...
(function (testing, core, jOrder) {
    ...
    test("Search", function () {
        ...
        equal(testing.table77.where([
            {'Total': 4}
        ], {renumber: true}).length, 10, "Counting rows where Total = 4");
        equal(testing.table77n.where([
            {'Total': 4}
        ], {renumber: true}).length, 10, "Counting rows where Total = 4 (NO INDEX)");

        // eaxact search on grouped field, non-packed
        equal(testing.table77.where([
            {'Total': 4}
        ]).length, 76, "Index of last row where Total = 4");
        equal(testing.table77n.where([
            {'Total': 4}
        ]).length, 76, "Index of last row where Total = 4 (NO INDEX)");
        ...
        deepEqual(testing.table77.where([
            {'ID': 76}
        ], {renumber: true}), expected, "Where ID = 76");
        deepEqual(testing.table77n.where([
            {'ID': 76}
        ], {renumber: true}), expected, "Where ID = 76 (NO INDEX)");

        // exact search on composite index, packed
        equal(testing.table77.where([
            {'Total': 5, 'Currency': 'USD'}
        ], {renumber: true, indexName: 'signature'})[0].ID, 11,
            "First suitable row by composite index (Total:5, Currency:USD)");
        equal(testing.table77n.where([
            {'Total': 5, 'Currency': 'USD'}
        ], {renumber: true})[0].ID, 11,
            "First suitable row by composite index (Total:5, Currency:USD, NO INDEX)");

        // range search
        equal(testing.table77.where([
            {'ID': {lower: 10, upper: 13}}
        ], {renumber: true, mode: jOrder.range}).length, 3,
        ...
        // text range search
        equal(testing.table1000.where([
            {'name': 'Using'}
        ], {renumber: true, mode: jOrder.startof}).length, 12,
            "Start of text search on text field");
        equal(testing.table1000n.where([
            {'name': 'Using'}
        ], {renumber: true, mode: jOrder.startof}).length, 12,
    ...
    test("Sorting", function () {
        ...
        deepEqual(tableX.orderby(['volumes'], jOrder.asc, {indexName: 'volumes'}), tableX.select([1, 2, 0
        ], {renumber: true}),
            "By numeric field, ascending");
        deepEqual(tableX.orderby(['volumes'], jOrder.asc, {indexName: 'volumes_uo'}), tableX.select([1, 2, 0
        ], {renumber: true}),
